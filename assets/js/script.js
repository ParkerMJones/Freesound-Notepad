...
var bgVideoLink = document.getElementById("bgVideoLink");
    ...
    .then(function (response) {
      ...
      fetch(
        "https://api.pexels.com/videos/search?query=" +
          response.results[0].tags[1] +
          "&per_page=20",
        {
          headers: {
            Authorization:
              "563492ad6f91700001000001423d0c460fd74c83a087ce29afa13898",
          },
        }
      )
        ...
        .then(function (videoResponse) {
          ...
          bgVideo.setAttribute(
            "src",
            videoResponse.videos[k].video_files[0].link
          );
          bgVideoLink.setAttribute("href", videoResponse.videos[k].url);
        });
...
notepad.addEventListener("input", (e) => {
  ...
  fetch(
    "https://freesound.org/apiv2/sounds/" +
      response.results[iGlobal].id +
      "?preview-hq-mp3&token=RqRsqgfKWUzssyVjBxkUg9ezWKNdZzqad7v4eKbe" /* 1st API Key: GafImFip5SoYm0xr01e4vWveTLlHqLcsHCVMlmTC */
  )
...
searchButton.addEventListener("click", function () {
  ...
  fetch(
    "https://freesound.org/apiv2/search/text/?query=" +
      ...
      "&page_size=47&page=" +
      randomPageNumber +
      "&fields=id,tags&token=RqRsqgfKWUzssyVjBxkUg9ezWKNdZzqad7v4eKbe"
  )
    .then(function (response2) {
      if (response2.status === 404) {
        ...
        setTimeout(function () {
          document.getElementById("search-container").removeChild(noResults);
        }, 2000);
      }
